...
const Editor = forwardRef(function Editor(props: Props, ref: React.ForwardedRef<EditorRefActions>) {
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      insertText: (content = "", prefix = "", suffix = "") => {
        ...
        const value = prevValue.slice(0, cursorPosition) + prefix + (content || prevValue.slice(cursorPosition, endPosition)) + suffix + prevValue.slice(endPosition);
  ...
  return (
    <div className={"flex flex-col justify-start items-start relative w-full h-auto bg-white" + className}>
      <textarea className=" mt-4 w-full h-full min-h-[60px] max-h-[300px] my-1 text-base resize-none overflow-x-hidden overflow-y-auto bg-transparent p-2 rounded-md outline-none whitespace-pre-wrap word-break" rows={1} placeholder={placeholder} ref={editorRef} onPaste={onPaste} onInput={handleEditorInput}></textarea>
    </div>
  );
});
